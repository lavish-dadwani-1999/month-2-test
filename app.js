...
const herokuapp ="https://corsanywhere.herokuapp.com";
...
// var post = fetchpost().then(posts => console.log(posts))
...
function display(item,wraper,rowspage,page) {
    ...
    for(let counter = 0; counter<=pagenated.length-1;counter++){
        ...
        div.addEventListener("click",function(){
            ...
            postbody.appendChild(div2);
            
            fetchcomments().then(data =>{
                const comment=data;

                comment.map(item =>{
                    if(userid==item.postId){
                        let div1 = document.createElement("div");
                        div1.classList.add("comments")
                        let p1 = document.createElement("p");
                        p1.innerText=item.body
                        div1.appendChild(p1);
...
function setpagenation(item,wraper,rowspage){
    wraper.innerHTML="";
    let pagecount = Math.ceil(item.length/rowspage)
    console.log(pagecount)
    for(let counter= 1;counter < pagecount+1;counter++){
        let btn = pagenationbutton(counter,item);
        wraper.appendChild(btn)
    }
}

function pagenationbutton(pages,item){
    ...
    button.addEventListener("click",function(){
        ...
        display(item,postlist,rows,currentpage);
        
        let currentbutton = document.querySelector(".pagenumber button.active ");
...
//    display(data,postlist,rows,currentpage)
// })

document.addEventListener("DOMContentLoaded",function(){
    fetchpost().then(function(data){
...
fetchpost().then(function(data){
    setpagenation(data,pagenumber,rows)
 })
